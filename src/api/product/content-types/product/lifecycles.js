const processEntity = async (entityId) => {
    ...
    return {
      productBenefitTags: updatedData?.productBenefitTags?.map(({ tag, bgColor }) => ({ tag, bgColor })) || [],
  ...
  const getAuthToken = async () => {
    const { STRAPI_AUTH_USER_ID, STRAPI_AUTH_PASSWORD, BASE_URL } = process.env;
  ...
  const updateProduct = async (processedData) => {
    ...
    const updateProductUrl = new URL(`${BASE_URL}/store/${STORE_ID}/strapi-sync`);
    return makeApiCall(updateProductUrl.toString(), 'POST', { apiKey: accessToken }, processedData);
  };
